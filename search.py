    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if child.state in explored:
                #aux_index = explored.index(child.state)
                pass
            elif child in frontier:
                aux_index = frontier.index(child)
                pass
        #------------------------------
...
def weighted_astar_search(problem, epsilon = None, h=None, display=False):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    h = memoize(h or problem.h, 'h')
    return best_first_graph_search(problem, lambda n: n.path_cost + (1+epsilon)*h(n), display)
